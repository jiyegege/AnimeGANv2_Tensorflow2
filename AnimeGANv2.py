...
class AnimeGANv2(object):
    def __init__(self, args):
        ...
        config_defaults = {
            'real_loss_weight': 1.7,
            'fake_loss_weight': 1.7,
            'gray_loss_weight': 1.7,
        ...
        config_defaults['d_lr'] = args.d_lr
        ...
        config_defaults['sty_weight'] = args.sty_weight
        ...
        print("# g_adv_weight,d_adv_weight,con_weight,sty_weight,color_weight,tv_weight : ", self.config.g_adv_weight,
              self.config.d_adv_weight, self.config.con_weight, self.config.sty_weight, self.config.color_weight,
              self.config.tv_weight)
        print("# init_lr,g_lr,d_lr : ", self.config.init_lr, self.config.g_lr, self.config.d_lr)
    ...
    def train(self):
        ...
        D_optim = Adam(self.config.d_lr, beta_1=0.5, beta_2=0.999)
        ...
        for epoch in range(start_epoch, self.config.epoch):
            ...
            if epoch >= self.config.init_epoch - 1:
                """ Result Image """
    ...
    def g_train_step(self, real, anime_gray, generated,
        ...
        with tf.GradientTape() as tape:
            ...
            tv_loss = self.config.tv_weight * total_variation_loss(fake_image)
            col_loss = color_loss(real, fake_image)
            t_loss = self.config.con_weight * c_loss + self.config.sty_weight * s_loss + col_loss * self.config.color_weight + tv_loss

            g_loss = self.config.g_adv_weight * generator_loss(self.config.gan_type, generated_logit)
    ...
    def d_train_step(self, real, anime, anime_gray, anime_smooth, generated, discriminator,
        ...
        with tf.GradientTape() as tape:
            ...
            d_loss = self.config.d_adv_weight * discriminator_loss(self.config.gan_type, d_anime_logit,
                                                                   d_anime_gray_logit,
                                                                   generated_logit,
                                                                   ...
                                                                   self.config.real_blur_loss_weight) + GP
    ...
    def model_dir(self):
        return "{}_{}_{}_{}_{}_{}_{}_{}_{}".format(self.model_name, self.dataset_name,
                                                   ...
                                                   int(self.config.con_weight), int(self.config.sty_weight),
